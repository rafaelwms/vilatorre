...
public class NegocioComum {
	...
	public Fornecedor consultarPorIdFornecedor(Integer id)throws Exception{
		...
		public void removerMateriaPrima(MateriaPrima materia)throws Exception{
			try {
					List<Ingrediente> lista = daoIngrediente.consultarTodos();
				
				for(Ingrediente ing : lista){
					
					if(ing.getMateriaPrima().getId() == materia.getId()){
						throw new Exception("Não é possível remover a materia prima "+materia.getNome()+", pois, está associada a um ou mais ingredientes.");
					}
				}
				
				Estoque estoqueMat = daoEstoque.localizarEstoqueMateria(materia);
				
				if(estoqueMat != null){
					daoEstoque.remover(estoqueMat);
				}
		...
		public void removerIngrediente(Ingrediente ingrediente)throws Exception{
			try {
				...
				List<Produto> lista = daoProduto.consultarTodos();
				
				for(Produto produto : lista){
					for(Ingrediente ing : produto.getIngredientes()){
						if(ing.getId() == ingrediente.getId()){
							throw new Exception("O ingrediente "+ingrediente.getNome()+" não pode ser removido, pois, está associado a um ou mais produtos existentes.");
						}
					}			
				}
		...
		public void removerProduto(Produto produto)throws Exception{
			try {
				for(ItemPedido item : daoItemPedido.consultarTodos()){
					if(item.getProduto().getId() == produto.getId()){
						throw new Exception("O produto "+produto.getNome()+" não pode ser removido, pois, é registrado em alguma venda.");
					}
				}
